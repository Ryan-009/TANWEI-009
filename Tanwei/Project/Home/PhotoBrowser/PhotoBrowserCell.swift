...
public class PhotoBrowserCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        //
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeTap(_:)))
    ...
    @objc func swipeTap(_ tap:UISwipeGestureRecognizer) {
        if  tap.state == .ended {
    ...
    @objc func onPan(_ pan: UIPanGestureRecognizer) {
        switch pan.state {
        case .began:
            if imageView.frame.width == 0 {//当获取不到imageView时 大小为0
                self.endPan()
                break
            }else{
