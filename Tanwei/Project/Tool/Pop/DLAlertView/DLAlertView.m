...
#define contentViewWidhtHeightRatio 1.0
...
-(void)dl_setupImageViewWithDiyView:(UIView *)diyView;
-(void)dl_setupDiyContentView;
@end

@implementation DLAlertView

-(instancetype)initWithDIYView:(UIView *)diyView clickCallBack:(ClickCallBack)clickCallBack andCloseCallBack:(CloseCallBack)closeCallBack;
{
    if (self = [super init]) {
        [self dl_setupNewWindow];
        [self dl_setupFrameNumber];
        [self dl_setupDiyContentView];
        [self dl_setupImageViewWithDiyView:diyView];
...
-(void)dl_setupDiyContentView
{
    UIView *contentView = [[UIView alloc] init];
    contentView.backgroundColor = [UIColor whiteColor];
    UITapGestureRecognizer *tapGresture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(text)];
    contentView.userInteractionEnabled = YES;
    contentView.clipsToBounds = YES;
    contentView.layer.cornerRadius = 5;
    [contentView addGestureRecognizer:tapGresture];
...
-(void)dl_setupImageViewWithDiyView:(UIView *)diyView
{
    ...
    diyView.frame = self.contentView.bounds;
    [self.contentView addSubview:diyView];
}
...
-(void)text{[self.view endEditing:true];}
